...
```{r code_folding = TRUE}
#load the data
ddos <- read_csv("./data/telegram/cumulative_observations.csv")
#summarize the data
options(scipen = 999)
summary(ddos)
```
...
```{r code_folding = TRUE}
#load the data
ddos <- read_csv("./data/telegram/active_observations.csv")
...
```{r code_folding = TRUE}
regional_observations <- read_csv("data/telegram/regional_observations.csv", 
...
```{r code_folding=TRUE}
ddos_final <- read_csv("data/telegram/cumulative_observations.csv")
ddos_df <- ddos_final %>% 
...
To make matching easier, I can use the "countrycode" package to assign proper country names to the ISO-3 numeric code from the data set.
...
Now I (may) want to clean up the rest of the subsetted data by assigning negative values to "0" when applicable. I've done this and saved it as a ".csv" file, so I'm going to clean up the current files and load that clean data.

```{r code_folding = TRUE}
rm(list = ls())
ivs_clean <- read.csv("./data/ivs/ivs_df_names.csv")
