...
Only includes full responses: EVS (2020b). European Values Study 2017: Integrated Dataset (EVS 2017). GESIS Data Archive, Cologne. ZA7500 Data file Version 4.0.0. doi:10.4232/1.13560.

EVS (2020c). European Values Study 2017: Integrated Dataset (EVS 2017)—Matrix Design Data. GESIS Data Archive, Cologne. ZA7502 Data file Version 2.0.0. doi:10.4232/1.13561.
...
There is some changes needed to make the data more manageable. I want to clean up some of the data by assigning negative values to "0" when applicable. The specific variables I'm going to look at first are from the 

```{r code_folding = TRUE}
ivs_important <- read.csv("ivs_means_important.csv")
ivs_important <- as_tibble(ivs_important)
names(ivs_important)[1] <- 'country'
head(ivs_important)
```
...
```{r code_folding = TRUE}
ivs_politics <- read.csv("ivs_means_politics.csv")
ivs_politics <- as_tibble(ivs_politics)
names(ivs_politics)[1] <- 'country'
head(ivs_politics)

politics <- ivs_politics %>%
  group_by(country) %>%
  summarise(
    willingness = mean(willing_fight),
    politics = mean(interest_politics),
    petition = mean(sign_petition),
    boycott = mean(join_boycott),
    demonstration = mean(attend_demonstration),
    strikes = mean(join_strikes),
    identity = mean(self_id)
  )
write.csv(politics, file = "politics.csv", row.names = TRUE)

fight <- ivs_politics %>%
  group_by(country) %>%
  mutate(willingness = case_when(
    willing_fight == 0 ~ "NA",
    willing_fight == 1 ~ "no",
    willing_fight == 2 ~ "yes"))
```