...
```{r code_folding = TRUE}
ddos_regions2 <- read_csv("ddos_regional2.csv", 
    col_types = cols(Date = col_date(format = "%m/%d/%Y")))
ddos_regions2 <- as_tibble(ddos_regions2) 
ddos_regions2
```

```{r code_folding = TRUE}

ggplot(ddos_regions2, aes(x = Date)) +
...
There are some changes needed to make the data more manageable. I have cleaned up some of the data by assigning all negative values representing various codes for no available observation to "NA" when applicable. I saved the resulting means by country as an offline data set that I will import.
